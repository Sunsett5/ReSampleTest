...
class DDIMSampler(object):
        ...
        for i_pass in outer_iterator:
            ...
            for i, step in enumerate(iterator):       
                ...
                img, _ = measurement_cond_fn(x_t=out, # x_t is x_{t-1}
                                                ...
                                                x_0_hat=pred_x0,
                                                scale=self.a_t*.5*80, # For DPS learning rate / scale
                                                )
            ...
            for i, step in enumerate(reversed(iterator)):        
                # Instantiating parameters
                index = (step+1)//2 - 1
                if i+1 < len(time_range):
                    prev_index = (time_range[i+1]+1)//2 - 1
    ...
    def forward_optimization(self, noisy_img, prev_img, cond, ts, index, quantize_denoised, score_corrector,
                             corrector_kwargs, unconditional_guidance_scale, unconditional_conditioning, lambda_=100.0, eps=1e-3, max_iters=100):
        ...
        for _ in range(max_iters):
            ...
            regularization_term = lambda_ * torch.mean(opt_var ** 2)
            ...
            optimizer.step()
            opt_var.data = torch.clamp(opt_var.data, -3.0, 3.0)  # Ensure the values are within [-1, 1]
    ...
    def p_sample_ddim(self, x, c, t, quantize_denoised=False, score_corrector=None, corrector_kwargs=None,
        ...
        dir_xt = (1. - self.a_prev**2).sqrt() * e_t
